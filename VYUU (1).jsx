...
function AuthScreen({ onAuth }) {
  ...
  const FINP={background:"#1A1A1A",border:"2px solid #2A2A2A",borderLeft:"2px solid #F2EFE9",color:"#F2EFE9",padding:"11px 14px",fontSize:13,fontFamily:"'Courier Prime',monospace",outline:"none",width:"100%",boxSizing:"border-box"};
  const LBL={fontSize:9,color:"#6A6260",textTransform:"uppercase",letterSpacing:"0.14em",marginBottom:6,fontFamily:"'Courier Prime',monospace",fontWeight:700};
  ...
  const submit = async () => {
    ...
    try {
      if(mode==="signup"){
        if(!name.trim()||!handle.trim()){setError("Name and username are required.");setLoading(false);return;}
        const {error:e} = await supabase.auth.signUp({
          email, password,
          options:{data:{name:name.trim(),handle:handle.trim().toLowerCase().replace(/\s/g,"")}}
        });
        if(e) throw e;
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#0D0D0D",display:"flex",flexDirection:"column",maxWidth:480,margin:"0 auto",fontFamily:"'Courier Prime',monospace"}}>
      <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",padding:"32px 28px"}}>
        {/* Logo */}
        <div style={{marginBottom:40,textAlign:"center"}}>
          <div style={{fontFamily:"'Rubik Dirt',sans-serif",fontSize:48,lineHeight:1,letterSpacing:"-0.02em",marginBottom:8}}>
            <span style={{color:"#FFE500"}}>VY</span><span style={{color:"#1440E8"}}>UU</span>
          </div>
          <div style={{fontFamily:"'Courier Prime',monospace",fontSize:9,color:"#4A4A4A",letterSpacing:"0.22em"}}>UNDERGROUND FASHION COMMUNITY</div>
        </div>

        {/* Mode toggle */}
        <div style={{display:"flex",borderBottom:"2px solid #2A2A2A",marginBottom:28}}>
          {[["signin","SIGN IN"],["signup","JOIN"]].map(([m,l])=>(
            <button key={m} onClick={()=>{setMode(m);setError("");}} style={{flex:1,background:"none",border:"none",borderBottom:mode===m?"3px solid #FFE500":"3px solid transparent",color:mode===m?"#F2EFE9":"#4A4A4A",padding:"10px 0",fontFamily:"'Archivo Black',sans-serif",fontSize:10,letterSpacing:"0.1em",cursor:"pointer",marginBottom:-2}}>{l}</button>
          ))}
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          {mode==="signup"&&(
            <>
              <div><div style={LBL}>Full Name</div><input style={FINP} value={name} onChange={e=>setName(e.target.value)} placeholder="Jordan Lee"/></div>
              <div><div style={LBL}>Username</div>
                <div style={{position:"relative"}}>
                  <span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",color:"#4A4A4A",fontSize:13}}>@</span>
                  <input style={{...FINP,paddingLeft:28}} value={handle} onChange={e=>setHandle(e.target.value.replace(/\s/g,"").toLowerCase())} placeholder="jordanlee"/>
                </div>
              </div>
            </>
          )}
          <div><div style={LBL}>Email</div><input style={FINP} type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com"/></div>
          <div><div style={LBL}>Password</div><input style={FINP} type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&submit()}/></div>
        </div>

        {error&&<div style={{marginTop:12,fontFamily:"'Courier Prime',monospace",fontSize:10,color:error.includes("Check your email")?"#1A6B3A":"#FF6B6B",lineHeight:1.6}}>{error}</div>}

        {/* Submit */}
        <button onClick={submit} disabled={loading} style={{marginTop:20,background:"#F2EFE9",border:"none",color:"#0D0D0D",padding:"14px",fontFamily:"'Archivo Black',sans-serif",fontSize:11,letterSpacing:"0.1em",cursor:loading?"not-allowed":"pointer",opacity:loading?0.6:1,boxShadow:"4px 4px 0 #FFE500"}}>
          {loading?"...":(mode==="signup"?"CREATE ACCOUNT →":"SIGN IN →")}
        </button>

        {/* Google */}
        <button onClick={signInGoogle} style={{marginTop:10,background:"none",border:"2px solid #2A2A2A",color:"#6A6260",padding:"12px",fontFamily:"'Courier Prime',monospace",fontSize:10,letterSpacing:"0.1em",cursor:"pointer"}}>
          CONTINUE WITH GOOGLE
        </button>
      </div>

      {/* Footer */}
      <div style={{padding:"16px 28px 32px",borderTop:"1px solid #1A1A1A",textAlign:"center"}}>
        <div style={{fontFamily:"'Courier Prime',monospace",fontSize:9,color:"#2A2A2A",letterSpacing:"0.14em"}}>BY JOINING YOU AGREE TO THE VYUU COMMUNITY GUIDELINES</div>
      </div>
    </div>
  );
}
...
function App() {
  ...
  if(authLoading) return <div style={{position:"fixed",inset:0,background:"#0D0D0D",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontFamily:"'Rubik Dirt',sans-serif",fontSize:32,color:"#FFE500"}}>VYUU</div></div>;
